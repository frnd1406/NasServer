...
func TestEncryptionPolicyService_DetermineMode_ForceUser(t *testing.T) {
	...
	assert.Equal(t, models.EncryptionUser, mode, "FORCE_USER override should return USER mode")

	mode = service.DetermineMode("test.pdf", 1024, "FORCE_USER")
	assert.Equal(t, models.EncryptionUser, mode, "FORCE_USER override should work for any file type")
}
...
func TestEncryptionPolicyService_DetermineMode_ForceNone(t *testing.T) {
	...
	assert.Equal(t, models.EncryptionNone, mode, "FORCE_NONE override should return NONE mode")

	mode = service.DetermineMode("passwords.xlsx", 1024, "FORCE_NONE")
	assert.Equal(t, models.EncryptionNone, mode, "FORCE_NONE should override policy for sensitive types")
}
...
func TestEncryptionPolicyService_DetermineMode_AutoWithSensitiveExtension(t *testing.T) {
	...
	testCases := []struct {
		filename     string
		expectedMode models.EncryptionMode
	}{
		// Sensitive documents should be encrypted
		{"contract.pdf", models.EncryptionUser},
		{"report.docx", models.EncryptionUser},
		{"spreadsheet.xlsx", models.EncryptionUser},
		{"presentation.pptx", models.EncryptionUser},

		// Security-critical files
		{"private.key", models.EncryptionUser},
		{"certificate.pem", models.EncryptionUser},
		{"keystore.p12", models.EncryptionUser},

		// Database files
		{"database.db", models.EncryptionUser},
		{"backup.sqlite", models.EncryptionUser},

		// Configuration files
		{".env", models.EncryptionUser},
		{"config.ini", models.EncryptionUser},
	}
...
func TestEncryptionPolicyService_DetermineMode_AutoWithNonSensitiveExtension(t *testing.T) {
	...
	for _, filename := range testCases {
		t.Run(filename, func(t *testing.T) {
			...
			assert.Equal(t, models.EncryptionNone, mode, "AUTO mode should NOT encrypt non-sensitive file: "+filename)
		})
	}
}
...
func TestEncryptionPolicyService_DetermineMode_HardwareLimit(t *testing.T) {
	...
	assert.Equal(t, models.EncryptionNone, mode, "Files exceeding hardware limit should not be encrypted")
	...
	assert.Equal(t, models.EncryptionUser, mode, "Files under hardware limit should follow policy")
}
...
func TestEncryptionPolicyService_DetermineMode_EmptyOverride(t *testing.T) {
	...
	assert.Equal(t, models.EncryptionUser, mode, "Empty override should default to AUTO mode")

	mode = service.DetermineMode("test.txt", 1024, "")
	assert.Equal(t, models.EncryptionNone, mode, "Empty override should default to AUTO mode")
}
...
func TestEncryptionPolicyService_DetermineMode_CaseInsensitive(t *testing.T) {
	...
	mode := service.DetermineMode("test.txt", 1024, "force_user")
	assert.Equal(t, models.EncryptionUser, mode, "Override should be case-insensitive")

	mode = service.DetermineMode("test.pdf", 1024, "Force_None")
	assert.Equal(t, models.EncryptionNone, mode, "Override should be case-insensitive")

	mode = service.DetermineMode("test.pdf", 1024, "auto")
	assert.Equal(t, models.EncryptionUser, mode, "AUTO should work in lowercase")
}
...
func TestEncryptionPolicyService_DetermineMode_ExtensionCaseInsensitive(t *testing.T) {
	...
	assert.Equal(t, models.EncryptionUser, mode1, ".PDF should be recognized")
	assert.Equal(t, models.EncryptionUser, mode2, ".pdf should be recognized")
	assert.Equal(t, models.EncryptionUser, mode3, ".Pdf should be recognized")
}
