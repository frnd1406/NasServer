...
func (b *BenchmarkService) RunStartupBenchmark() error {
	...
	warmupBuf := make([]byte, 0, len(testData)+TagSize)
	for i := 0; i < b.warmupRounds; i++ {
		warmupBuf = aead.Seal(warmupBuf[:0], nonce, testData[:ChunkSize], nil)
	}

	// Pre-allocate output buffer
	outputBuf := make([]byte, 0, len(testData)+TagSize)
	...
	writer := &bytes.Buffer{}
	writer.Grow(int(b.testSizeBytes) + HeaderSize + (int(b.testSizeBytes)/ChunkSize+1)*TagSize)
	...
	for {
		...
		deriveChunkNonceInPlace(nonce, chunkIndex, chunkNonce)
