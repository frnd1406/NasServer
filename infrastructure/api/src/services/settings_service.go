...
func (s *SettingsService) UpdateBackupSettings(ctx context.Context, settings BackupSettingsDTO) error {
	...
	err := s.settingsRepo.UpsertMany(ctx, map[string]string{
		repository.SystemSettingBackupSchedule:  schedule,
		repository.SystemSettingBackupRetention: fmt.Sprintf("%d", settings.Retention),
		repository.SystemSettingBackupPath:      path,
	})
	if err != nil {
