...
func NewContentDeliveryService(storage *StorageManager, encryptionSvc *EncryptionService, logger *logrus.Logger) *ContentDeliveryService {
...
func (s *ContentDeliveryService) GetStream(ctx context.Context, path string, rangeHeader string, password string, mode string, user *models.User) (*FileStreamResult, error) {
...
func (s *ContentDeliveryService) detectEncryptionStatus(fullPath string) models.EncryptionMode {
	// Check file extension first (fast path)
	if strings.HasSuffix(strings.ToLower(fullPath), ".enc") {
		// Verify with magic bytes
		isEnc, err := IsEncryptedFile(fullPath)
		if err != nil {
...
func (s *ContentDeliveryService) prepareEncryptedStream(fullPath string, fileInfo os.FileInfo, rangeHeader string, password string) (*FileStreamResult, error) {
	...
	encInfo, err := GetEncryptedFileInfo(file)
	if err != nil {
		file.Close()
	...
	if rangeHeader != "" {
		...
		go func() {
			...
			_, err := DecryptStreamWithSeek(password, file, w, start, length)
			if err != nil {
		...
		go func() {
			...
			file.Seek(0, 0)
			err := DecryptStream(password, file, w)
			if err != nil {
				s.logger.WithError(err).Error("Async decryption failed")
				w.CloseWithError(err)
			}
