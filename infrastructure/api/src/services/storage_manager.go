...
type SaveResult struct {
	...
	Checksum         string
	StoragePath      string
	EncryptionStatus models.EncryptionMode
...
func (s *StorageManager) SaveWithEncryption(
	...
	var encMeta *models.EncryptionMetadata
	...
	go func() {
		...
		if mode == models.EncryptionNone {
		...
		} else if mode == models.EncryptionUser {
			...
			if err := EncryptStream(password, file, multiWriter); err != nil {
				errChan <- err
				return
			}

			// Set metadata
			encMeta = &models.EncryptionMetadata{
	...
	if s.fileRepo != nil {
		...
		if result.EncryptionMeta != nil {
			...
			dbFile.EncryptionMetadata = &models.EncryptionMetadataJSON{EncryptionMetadata: *result.EncryptionMeta}
		}
...
func (s *StorageManager) Save(dir string, file multipart.File, fileHeader *multipart.FileHeader) (*SaveResult, error) {
	return s.SaveWithEncryption(context.Background(), dir, file, fileHeader, models.EncryptionNone, "")
}
